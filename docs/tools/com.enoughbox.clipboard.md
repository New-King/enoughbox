...
快捷键呼出浮窗，支持搜索、分类（最近 / 全部 / 文本 / 图片 / 其他）、列表选用、复制与删除。默认点击条目或回车会粘贴到打开浮窗前的应用并关闭浮窗；钉住后，点击与回车只粘贴、不关窗。钉住状态会记住。行内复制图标只写入系统剪贴板，并在浮窗顶部提示「已复制到剪贴板」。再次按快捷键或 `Esc` / `⌘W` / `⌘Q` / 关闭按钮可关掉浮窗。自动粘贴需要辅助功能权限。
...
5. 点左上角图钉后，再点击或回车应粘贴且窗口不关；再点图钉取消。行内复制只写入剪贴板并在浮窗顶部提示「已复制到剪贴板」。删除条目后不再出现。`Esc` / 快捷键仍可关闭。
6. 工具「移除」后热键失效、监听停止。
