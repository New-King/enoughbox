...
快捷键呼出浮窗，支持搜索、分类（最近 / 全部 / 文本 / 图片 / 其他）、列表选用、复制与删除。点击条目会粘贴到打开浮窗前的应用，浮窗保持打开；行内复制图标只写入系统剪贴板并提示「已复制到剪贴板」。再次按快捷键或 `Esc` / `⌘W` / `⌘Q` 关闭。自动粘贴需要辅助功能权限。
...
5. 点行内复制图标，系统剪贴板应变为该条，并出现「已复制到剪贴板」；再点删除，条目消失。再次按快捷键可关闭浮窗。
6. 工具「移除」后热键失效、监听停止。
